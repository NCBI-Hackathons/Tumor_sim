    ...
    def get_location_on_sequence(self, seq, distribution='uniform'):
        if distribution == 'uniform':
            ...
            while True:
                ...
                if seq[location] != 'N':
                    return location
        elif distribution == 'normal' or 'gaussian':
            # Don't select a location with a N, and discard draws outside of bounds
            while True:
                mean = len(seq)/2
                sigma = len(seq)/6
                location =  np.random.normal(mean, sigma, 1)
                if location < 0 or location > length(seq) or seq[location] != 'N':
                    return location
        else:
            raise NotImplementedError("Only Uniform & Gaussian are implemented!")
